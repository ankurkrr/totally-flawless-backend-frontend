...
// Removed client-side AWS SDK imports and Buffer usage. Uploads are proxied
// to the backend via `uploadToS3` which posts multipart/form-data to the server.
import axios from 'axios';
import AsyncStorage from '@react-native-async-storage/async-storage';
import { Icon } from '@rneui/base';
import AIcons from 'react-native-vector-icons/Feather';
...
const ArtistProfile = ({ navigation }) => {
  ...
  const [videoUrlCopy, setVideoUrlCopy] = useState("")
  ...
  // console.log('drivingLicence', drivingLicence)
  ...
  const getData = async () => {
    ...
    try {
      ...
      // {
      //   "address": "Chakan ", "businessType": 2, "city": "null", "countryCode": "91", "createdDate": "2025-01-06T08:42:24.000Z",
      //     "email": "mahesh@gmail.com", "facebook": "facebook.com", "firstName": "Mahesh",
      //       "geocode": "null", "id": "273a254d-a2d6-4272-bfc8-7001c761a84e", "instagram": "instagram.com",
      //         "isApproved": 1, "isVideoUploaded": 1, "lastName": "Test", "lastUpdatedOn": "2025-01-06T11:48:24.000Z",
      //           "licenceUrl": "https://mmobileapp.s3.ap-south-1.amazonaws.com/flawless/rn_image_picker_lib_temp_72b27686-68b3-422d-982f-f3e00cedb08e.jpg",
      //             "mobile": "9975464417", "otp": 1111, "sin": "123852033", "state": "maharashtra", "videoUrl": "null"
      // }
      const imgTemp = data?.artistData?.map((item: any) => {
        return item?.type == "image" ? item?.url : ""
      }).filter((item: any) => item != null && item !== "");
      const videoTemp = data?.artistData?.map((item: any) => {
      ...
      temp.images = imgTemp
      temp.videoUrl = videoTemp[0] || ""
      temp.sin = data?.sin=="null"?"":data?.sin
      temp.facebook = data?.facebook=="null"||!data?.facebook?"":data?.facebook
      temp.instagram = data?.instagram=="null"||!data?.instagram?"":data?.instagram
      temp.licenceUrl = data?.licenceUrl || ""
      temp.mobile = data?.mobile
      temp.countryCode = data?.countryCode||"";
      temp.profileImage = data?.profileImage;
      setProfileImgUri(data?.profileImage || "")
      setArtistData(data)
      setSelected(bTypes[data?.businessType - 1].key)
      setDrivingLicence(temp?.licenceUrl || "")
      setMobileNo(data?.mobile)
      setFormData(temp)
      setImageUris(imgTemp)
  ...
  const validateForm = () => {
    ...
    const trimmedSSN = formData.sin ? formData.sin.trim() : "";
    ...
    // if (!trimmedFacebook) {
    //   formErrors.facebook = 'Facebook URL is required';
    //   // } else if (!urlRegex.test(trimmedFacebook)) {
    // } else
      ...
      launchCamera(options, async (response: any) => {
        ...
        if (response.didCancel) {
          ...
          if (currentImg == 'imageUrl') {
            ...
            let newImageUris: any = [...imageUris];
    ...
    try {
      ...
      launchImageLibrary(options, async (response: any) => {
        ...
        if (response.didCancel) {
          ...
          console.log('Response = ', response);
          let newImageUri: any = response.uri || response.assets?.[0]?.uri;
          if (currentImg == 'imageUrl') {
            console.log('imageUrl');
            let newImageUris: any = [...imageUris];
          ...
          if (currentImg == 'imageUrl') {
            console.log('imageUrl');
            let formImages: any = [...(formData.images || [])];
            formImages.push(imgUrl);
            setFormData({
              ...formData,
              images: formImages,
  ...
  const SignUpApi = async () => {
    ...
    try {
      ...
      //   formErrors.video = 'Video Required';
      ...
      if (response.status === 200) {
        ...
        setIsLoading(false);
        showToast("Artist infomation has been updated successfully.")
        navigation.goBack();
      }
      setIsLoading(false);
    ...
    <View style={{ backgroundColor: '#FFF', flex: 1 }}>
      ...
      <KeyboardAwareScrollView
        ...
        <View style={GlobalStyles.headerTitleView} >
          <TouchableOpacity style={GlobalStyles.backBtn} onPress={() => changeNavigation(screenNames.ARTIST_HOME)} >
            <CommonSvg.back />
          </TouchableOpacity>
        ...
        <ScrollView contentContainerStyle={{ paddingBottom: mvs(50) }} style={{ backgroundColor: '#FFF' }}>
          <Modal
            transparent={true}
...
<TouchableOpacity
                        ...
                        onPress={() => {
                            ...
                            if (profileImgUri) {
                                ...
                                setImageData([{ uri: profileImgUri }])
                            } else {
                                toggleProfileImg()
                            }
                    ...
                    <TouchableOpacity onPress={() => {
                        ...
                        <Text style={[GlobalStyles.txtM14Dark, { color: COLORS.blue,textAlign:'center' }]} >Edit Picture</Text>
                    </TouchableOpacity>

                    <View>
                        {errors?.imgUrl && (
                            <Text style={{...styles.errorText,textAlign:'center'}}>{errors.imgUrl}</Text>
                        )}
                    </View>
                ...
                <View
                  ...
                  <Text style={{ fontSize: 14, fontWeight: '400', color: '#000000' }}>
                    Uploaded Images of your past work.
                  </Text>
                </View>
                ...
                <View
                  ...
                  <Text style={{ fontSize: 14, fontWeight: '400', color: '#000000' }}>
                    Uploaded Short Video Presentation.
                  </Text>
                  <Icon
                ...
                </View>
                {
                  formData.videoUrl ?
                    <TouchableOpacity
                      ...
                      <TouchableOpacity onPress={()=> toggleOverlay('videoUrl', 'video')} style={{ height: 75, width: 75, ...GlobalStyles.alignJustifyCenter }} >
                        <Image
                          source={require('../../assets/uploadImageIcon.png')}
                          style={{ height: 75, width: 75 }}
                        />
                      ...
                      <View style={{ height: 100, width: '50%', backgroundColor: COLORS.black, borderRadius: 6, alignItems: 'center', justifyContent: 'center', }} >
                        <AIcons name='play-circle' color={COLORS.white} size={35} />
                      </View>
                      {/* <TouchableOpacity style={{position:'absolute',top:12,right:35}} >
                      <ANIcons
                                color={COLORS.white}
                                name="closecircleo"
                                size={18}
      ...
      {
        artistData?.isApproved != 0 &&
        <ArtistBottomBar navigation={changeNavigation} page={screenNames.ARTIST_PROFILE} />
      }
