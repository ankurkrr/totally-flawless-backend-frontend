import {
  ...
  import { CART_DATA, CART_ITEMS, IS_ADDED_CART, IS_ORDER_ACCEPTED } from '../store/allactionsTypes';
  ...
  const Cart = ({ navigation }) => {
    ...
    const [reqArtistCount, setReqArtistCount] = useState({
      hair: 0,
      makeup: 0
    })
    ...
    const [artistAcceptTimer, setArtistAcceptTimer] = useState(0)
    const [notificationSentArtistIndex, setNotificationSentArtistIndex] = useState(0)
    const [hairNotificationSentArtistIndex, setHairNotificationSentArtistIndex] = useState(0)
    const [makeupNotificationSentArtistIndex, setMakeupNotificationSentArtistIndex] = useState(0)
    ...
    const getUserDetails = async (id) => {
      ...
      // );
      // console.log('updateUser >>>>', response.data.data[0]);
    ...
    const handleSendNotificationToUser = async () => {
      ...
      console.log('acceptedArtist user>>>', acceptedArtist[0])
  
      return acceptedArtist[0];  // Return the final data
    };
    ...
    useEffect(() => {
      ...
      const subscribe = setTimeout(() => {
        if (artistAcceptTimer <= 0 && isNotificationSended) {
          setArtistAcceptTimer(0)
          if (isSeperateSend) {
            getHairArtistByBookingId();

            setTimeout(() => {
              getMakeupArtistByBookingId()
              setArtistAcceptTimer(40)
            }, 10000);
          } else {
          ...
          setArtistAcceptTimer(prev => { return prev - 1 });
        }
      }, 2000);
    ...
    const getArtistByBookingId = async () => {
      try {
        ...
        //handle accepted artist flow
        if (acceptedArtists?.length == finalCount) {
        ...
        //When both skill artist no one accepted request
        if (availableArtists.length <= notificationSentArtistIndex) {
          handleSeperateArtistRequest()
        }
        setIsNotificationSended(true);
        setTimerModalVisible(true);
        sendNotifications(finalCount, availableArtists, groupedItems);
    ...
    const handleSeperateArtistRequest = () => {
      setIsSeperateSend(true);
      setTimerModalVisible(true);
      setHairNotificationSentArtistIndex(0);
      setMakeupNotificationSentArtistIndex(0);
      const [hairCount, makeCount] = returnRequestedArtistCount();
      ...
      if (makeCount > 0) {
        setTimeout(() => {
          ...
          setArtistAcceptTimer(40)
          setIsNotificationSended(true)
        }, 10000);//delay to 10 sec
      }
    }
  
  
  
    const sendNotifications = (finalCount, artistResponse, sendCartItems) => {
      ...
      const data = {
        totalAmount,
        bookingFee: cartType == "later" ? 0 : parseInt(70 / finalCount),
        artistBookingCount: finalCount,
        userToken: fcmToken || "", // accept send notification
        cartDetails: sendCart,
        artistDetails: artistResponse[notificationSentArtistIndex],
        cartItems: sendCartItems[0],
        ...artistResponse,
      };
  
      handleNotification(data, artistResponse[notificationSentArtistIndex]);
    ...
    const handleAcceptedArtists = (acceptedArtists, finalCount) => {
      ...
      if (finalCount === uniqueArtists.length) {
    ...
    const getHairArtistByBookingId = async () => {
      try {
        ...
        const url = `/usertoartistlocation?userId=${id}&userCartId=${cartId}&businessType=${businessType}`;
        console.log("url", url);
        console.log("finalCount getHairArtistByBookingId >>>", finalCount);
        ...
        //handle accepted artist flow
        if (acceptedArtists?.length == hairCount) {
          const acceptedData = [...acceptedArtist]
          acceptedData.push(acceptedArtists)
          setAcceptedArtist(acceptedData)
          const flatMapData = acceptedData?.flatMap(item => item?.bookingDetails || []);
  
          handleAcceptedArtists(flatMapData, finalCount)
  
          return;
  
        }
  
        //When hair skill artist no one accepted request
        if (availableArtists.length <= hairNotificationSentArtistIndex) {
          // handleSeperateArtistRequest()
          return
        }
        sendNotificationsHair(finalCount, availableArtists, groupedItems);
    ...
    const sendNotificationsHair = (finalCount, artistResponse, sendCartItems) => {
      ...
      const data = {
        ...
        userToken: fcmToken || "", // accept send notification
        cartDetails: sendCart,
        artistDetails: artistResponse[hairNotificationSentArtistIndex],
        cartItems: sendCartItems[0],
        ...artistResponse,
      };
  
      handleNotificationHair(data, artistResponse[hairNotificationSentArtistIndex]);
    ...
    const handleNotificationHair = debounce(async (sendData, data) => {
      ...
      setHairNotificationSentArtistIndex(hairNotificationSentArtistIndex + 1)
      setArtistAcceptTimer(40)
    }, 3000); // 3 seconds debounce delay
    ...
    const getMakeupArtistByBookingId = async () => {
      try {
        ...
        const availableArtists = data?.availableArtists?.length == 0 ? [] : data?.availableArtists;
        const acceptedArtists = data?.acceptedArtists.length == 0 ? [] : data?.acceptedArtists;
        // console.log(data?.availableArtists?.length === 0 && acceptedArtists?.length == 0)
        ...
        //handle accepted artist flow
        if (acceptedArtists?.length == makeCount) {
        ...
        //When hair skill artist no one accepted request
        if (availableArtists.length <= makeupNotificationSentArtistIndex) {
          // handleSeperateArtistRequest()
          return
        }
        sendNotificationsMakeup(finalCount, availableArtists, groupedItems);
      } catch (error) {
        handleErrorMakeup(error);
      }
    };
  
    const sendNotificationsMakeup = (finalCount, artistResponse, sendCartItems) => {
      ...
      const data = {
        ...
        userToken: fcmToken || "", // accept send notification
        cartDetails: sendCart,
        artistDetails: artistResponse[makeupNotificationSentArtistIndex],
        cartItems: sendCartItems[0],
        ...artistResponse,
      };
  
      handleNotificationMakeup(data, artistResponse[makeupNotificationSentArtistIndex]);
  
    };
  
    const handleNotificationMakeup = debounce(async (sendData, data) => {
      ...
      setMakeupNotificationSentArtistIndex(makeupNotificationSentArtistIndex + 1)
      setArtistAcceptTimer(40)
    }, 3000); // 3 seconds debounce delay
    ...
    const handleErrorMakeup = (error) => {
      if (error?.response?.status === 404) {
        ...
        console.error("error handleErrorMakeup >>>", error);
      }
    };
    ...
    //   const token = await generateJWT(fcmService)
    //   const accesstoken = await fetchAccessToken(token, "https://oauth2.googleapis.com/token")
  
    //   const requestData = {
    //     customData: JSON.stringify(sendData)
    //   }
    //   const isSended = await sendNotification(receiver, accesstoken, "Booking request received!", "Tap to open app and accept request", requestData)
    //   console.log('isSended >>>>', isSended)
    //   if (!isSended) {
    //     const temp = [...rejOrErrorSentArtists]
    //     temp.push(data?.artistId)
    //     setRejOrErrorSentArtists(temp);
    //     const temp1 = [...notificationSentArtists]
    //     temp1.push(filterArtist[0]?.artistId)
    //     setNotificationSentArtists(temp);
    //   }
    // }, 5000); // 3 seconds debounce delay
    // // }
              ...
              <ScrollView showsVerticalScrollIndicator={false}>
                ...
                <View
                  ...
                  <Text
                    ...
                    *Each service will take approximately 1hr.
                  </Text>
                </View>
              ...
              <View style={styles.modalContainer}>
                <View style={[styles.modalContent, { minHeight: Platform.OS == "ios" ? mvs(250) : mvs(305) }]}>
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() => setCheckOutSummaryModal(false)}>
