/**
 * Run Key Management Schema Migration
 * 
 * Executes the key management table creation SQL
 */
...
async function runMigration() {
    console.log('🔄 Running key management schema migration...\n');

    // Read SQL file
    const sqlPath = path.join(__dirname, '..', 'DB', 'key_management_schema.sql');
    ...
    try {
        ...
        console.log('   Tables created: api_keys, key_rotation_history, key_usage_logs');
        ...
        if (tables.length > 0) {
            ...
            const [columns] = await connection.query('DESCRIBE api_keys');
